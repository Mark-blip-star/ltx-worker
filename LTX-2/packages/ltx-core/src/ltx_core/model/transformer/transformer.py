...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def _should_use_tgate_cache(self) -> bool:
        start = os.environ.get("LTX_TGATE_START_STEP")
        if not start:
            return False
        try:
            return int(os.environ.get("LTX_CURRENT_STEP_INDEX", "-1")) >= int(start)
        except ValueError:
            return False
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_vx:
            ...
            if self._should_use_tgate_cache() and hasattr(self, "_tgate_video_delta"):
                vx = vx + self._tgate_video_delta
            else:
                v_text_delta = self._apply_text_cross_attention(
                    vx,
                    video.context,
                    self.attn2,
                ...
                self._tgate_video_delta = v_text_delta.detach()
                vx = vx + v_text_delta
        ...
        if run_ax:
            ...
            if self._should_use_tgate_cache() and hasattr(self, "_tgate_audio_delta"):
                ax = ax + self._tgate_audio_delta
            else:
                a_text_delta = self._apply_text_cross_attention(
                    ax,
                    audio.context,
                    self.audio_attn2,
                ...
                self._tgate_audio_delta = a_text_delta.detach()
                ax = ax + a_text_delta
        ...
        if run_a2v or run_v2a:
            ...
            if run_v2a and not audio.cross_attn_skip_all:
                ...
                del gate_out_v2a, ax_scaled, vx_scaled
