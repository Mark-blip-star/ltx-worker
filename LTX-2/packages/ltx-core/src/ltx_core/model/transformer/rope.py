...
_FREQ_GRID_DEVICE_CACHE: dict[tuple[int, tuple[int, ...], torch.dtype, str], torch.Tensor] = {}
...
def generate_freqs(
    ...
    if indices.device != fractional_positions.device:
        cache_key = (indices.data_ptr(), tuple(indices.shape), indices.dtype, str(fractional_positions.device))
        cached_indices = _FREQ_GRID_DEVICE_CACHE.get(cache_key)
        if cached_indices is None:
            cached_indices = indices.to(device=fractional_positions.device)
            _FREQ_GRID_DEVICE_CACHE[cache_key] = cached_indices
...
def precompute_freqs_cis(
    ...
    rope_type: LTXRopeType = LTXRopeType.SPLIT,
    freq_grid_generator: Callable[[float, int, int, torch.device | None], torch.Tensor] = generate_freq_grid_pytorch,
