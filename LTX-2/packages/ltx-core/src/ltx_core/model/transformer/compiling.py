...
def compile_transformer(model: LTXModel) -> LTXModel:
    model.transformer_blocks = torch.nn.ModuleList(torch.compile(m, mode="reduce-overhead") for m in model.transformer_blocks)

    def patched_dynamo_forward(*args, **kwargs) -> tuple[torch.Tensor, torch.Tensor]:
        with ExitStack() as stack:
            if hasattr(torch._inductor.config, "unsafe_skip_cache_dynamic_shape_guards"):
                stack.enter_context(torch._inductor.config.patch(unsafe_skip_cache_dynamic_shape_guards=True))
            stack.enter_context(
