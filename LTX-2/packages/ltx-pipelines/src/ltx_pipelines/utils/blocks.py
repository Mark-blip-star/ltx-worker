...
class DiffusionStage:
    ...
    def _build_transformer(self, *, device: torch.device | None = None, **kwargs: object) -> X0Model:
        ...
        load_device = torch.device("cpu") if os.environ.get("LTX_CPU_STAGE_LOAD") == "1" else target
        model = X0Model(builder.build(device=load_device, **kwargs))
        if os.environ.get("LTX_REPORT_STAGE_MEMORY") == "1":
            resident_bytes = sum(p.numel() * p.element_size() for p in model.parameters())
            logger.info(
                "Stage transformer resident weights before transfer: %.3f GiB; CUDA already allocated: %.3f GiB",
...
class VideoDecoder:
    ...
    def __init__(
        ...
        self._dtype = dtype
        self._device = device
        memory_efficient = memory_efficient and os.environ.get("LTX_FAST_VAE") != "1"
