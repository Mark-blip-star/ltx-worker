...
const specPath = path.join(here, 'terminal_keyframe_screening_10s_v1.json');
const spec = JSON.parse(fs.readFileSync(specPath, 'utf8'));

assert.strictEqual(
  spec.experimentId,
  'terminal-keyframe-stage1-screening-10s-v1',
);
assert.strictEqual(spec.cases.length, 4);
assert.strictEqual(spec.defaults.frames, 241);
assert.strictEqual(spec.defaults.terminal_keyframe_strength_stage1, 0.25);
assert.strictEqual(spec.defaults.terminal_keyframe_strength_stage2, 0);
assert.strictEqual(spec.requireExtraLora, false);
